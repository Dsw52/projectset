...
Nut_by_state_bar <-ggplot(A_macros_sum_noP, aes(x = StateID, y = Total_Abundance, fill = Nutrient)) +
  geom_bar(stat = "identity", .groups = "drop") +
  labs(title = "Total Nutrient Abundance Across States",
       x = "State",
       y = "Total Abundance (% weight)",
...
nutrient_ls_bar <- ggplot(A_macros_sum_noP, aes(x = Nutrient, y = Total_Abundance, fill = Nutrient))+
  geom_bar(stat = "identity", .groups = "drop")+
  facet_wrap(vars(LandCover1), nrow = 2)+
  theme_bw()+ 
  labs(x = "Nutrient",
       y = "Total Sum (% Weight)",
       title = "Soil Macronutrient Totals by Land Status Type (excluding")+
  scale_fill_viridis(discrete = TRUE)
nutrient_ls_bar
```
