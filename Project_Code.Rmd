...
```{r}
# Define the coordinates (e.g., New York City)
lat <- 40.7128
lon <- -74.0060

crs(prism_t)

point_wgs84 <- vect(cbind(lon, lat), crs = "EPSG:4326")
crs(point_wgs84)
point_nad83 <- project(point_wgs84, crs(prism_t))

# Create a spatial point
point <- c(lon, lat)

# Extract the value at the point from the raster
value_at_point <- extract(prism_t, point_nad83)

# Print the extracted value (e.g., temperature, precipitation, etc.)
